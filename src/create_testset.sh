...
treebank="../data/treebanks/es-ud-train.conllu"
lang="English"
feature="Number"
...
mkdir -p tmp/$lang/$feature

# extracting word forms, lemmas and morphological features
echo "== collect paradigms =="
python data/collect_paradigms.py --input $treebank --output tmp/$lang/$feature/paradigms.txt --min_freq 0

echo "== extract agreement constructions =="
# extracting patterns which correspond to agreement relation
## with vocab
python syntactic_testsets/extract_dependency_patterns.py --treebank $treebank --output tmp/$lang/$feature --features $feature --vocab $lm_data/vocab.txt --paradigms tmp/$lang/$feature/paradigms.txt

## without vocab
# python syntactic_testsets/extract_dependency_patterns.py --treebank $treebank --output tmp/$lang/$feature --features $feature --paradigms tmp/$lang/$feature/paradigms.txt
...
#                                                --output ../data/agreement/$lang/generated_by_myself \
